...
public class MessageToEntityMapper {
    ...
    public InboundMessageEntity mapMessageToEntity(Exchange exchange, IchicsrMessage icsr) throws Exception {
        ...
        if (icsr.getSenderid() == null ||
            ...
            LOG.error(
                    "Message not persisted as the mandatory icsr message headers have invalid value - senderId = {} , recevied date = {}",
                    icsr.getSenderid(), icsr.getMessagedate());
            //throw exception to rollback
            throw new InvalidMessageHeaderException("Message " + icsr.getMessagenumber()
                    + "not persisted as the mandatory icsr message headers have invalid value");
        }
        ...
        entity.setReceivedate(
                (Date) exchange.getIn().getHeader(MessageConstants.MESSAGE_RECEIVED_DATE));//icsr.getMessagedate());
        entity.setSenderid(icsr.getSenderid());
        entity.setStatus(exchange.getIn().getHeader(MessageConstants.MESSAGE_HEADER_VALIDATIONRESULT).toString()
                .equalsIgnoreCase(MessageConstants.MESSAGE_HEADER_VALID) ? "Y" : "N");
        ...
        try {
            ...
            LOG.warn("Message Box for the receiver= {} not available, new one will be created; Exception: {}",
                    icsr.getReceiverid(), e.getMessage()); //no data found exception
        }
