 ...
 * hangout because the connection thread is not going to be close <br/>
 * <br/>
...
public class LogStashSocketAppender extends AppenderBase<ILoggingEvent> implements Runnable,
    ...
    protected void append(ILoggingEvent event) {
        ...
        try {
            ...
            if (!inserted) {
                addInfo("Dropping event due to timeout limit of [" + eventDelayLimit + "] milliseconds being exceeded");
            }
        }
    ...
    private void dispatchEvents() throws InterruptedException {
        ...
        try {
            ...
            while (true) {
                ...
                outputStream.flush();
                if (event.getMarker() != null && event.getMarker().contains(ClassicConstants.FINALIZE_SESSION_MARKER)) {
                    break;
                }
