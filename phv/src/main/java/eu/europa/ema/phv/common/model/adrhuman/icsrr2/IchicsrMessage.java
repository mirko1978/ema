...
public class IchicsrMessage implements Serializable {
     ...
     */
    @Id
    @GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "IchicsrMessage")
    @SequenceGenerator(name = "IchicsrMessage", sequenceName = "SEQ_ICHICSRMESSAGE", allocationSize = 1)
    ...
    /**
     * This field is obsolete. Now they are using document type.<br/>
     * <code>select MESSAGETYPE, count(*) from I_ICHICSRMESSAGE group by MESSAGETYPE;</code><br/>
     * on June 2014<br/>
     * <pre>
     * TYPE     COUNT
     *  1	    48730
     ...
     */
    @OneToOne(cascade = CascadeType.ALL, mappedBy = "IIchicsrmessage")
    @XmlTransient
    private MessageAck IMessageack;

    // bi-directional many-to-one association to SafetyReports
    @OneToMany(mappedBy = "ichicsrMessage", cascade = CascadeType.ALL)
    ...
    public void initializeForeigners() {
        if (safetyReports != null) {
            for (SafetyReports s : safetyReports) {
                s.setFkIchicsrmessage(pkIchicsrmessage);
            }
        }
    }
